...
from tabs.parameters.settings import KB_MODEL_ID, get_bisheng_settings
from tabs.parameters.summaries import aggregate_outputs, persist_compare_outputs, summarize_with_ollama
from tabs.parameters.workflow import PARAMETERS_WORKFLOW_SURFACE
...
def run_parameters_job(
    session_id: str,
    publish: Callable[[Dict[str, object]], None],
    check_control: Optional[Callable[[], Dict[str, bool]]] = None,
) -> Dict[str, List[str]]:
    """Run the parameters workflow headlessly and report progress via ``publish``."""
    ...
    if graph_dir and os.path.isdir(graph_dir):
        ...
        process_archives(graph_dir, examined_txt_dir, emitter)
    ...
    kb_name_dyn = PARAMETERS_WORKFLOW_SURFACE.knowledge_base_name(session_id)
    kid = None
    try:
    ...
    try:
        warmup_prompt = PARAMETERS_WORKFLOW_SURFACE.warmup_prompt or "预热：请回复'参数检查准备就绪'。"
        call_flow_process(
            base_url=settings.base_url,
            ...
            session_id=None,
            history_count=0,
            extra_tweaks=settings.flow_tweaks,
    ...
    for name in sorted(exam_txt_files, key=lambda value: value.lower()):
        ...
        for index, piece in enumerate(chunks, start=1):
            ...
            log_llm_metrics(
                ...
                session_id,
                {
                    "ts": datetime.now().isoformat(timespec="seconds"),
                    "engine": "bisheng",
                    "model": f"parameters_flow:{settings.flow_id or 'unknown'}",
        ...
        try:
            ...
            summarize_with_ollama(initial_results_dir, parameters_out_root, session_id, name_no_ext, full_out_text)
        except Exception as error:
            report_exception(f"保存比对结果失败({name})", error, level="warning")

    emitter.set_stage("aggregate")
    try:
        aggregate_outputs(initial_results_dir, parameters_out_root, session_id)
    except Exception as error:
