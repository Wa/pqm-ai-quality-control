"""Configuration helpers for the parameters check workflow."""
...
TAB_ENV_PREFIX = "PARAMETERS_CHECK"
TAB_SLUG = "parameters"
KB_MODEL_ID = 7
...
def _bisheng_setting(
    ...
    *,
    ...
    tab_config = bisheng_config.get("tabs", {}).get("parameters_check", {})
...
def _sanitize_base_url(raw: str) -> str:
    if not raw:
        return raw
    trimmed = raw.rstrip("/")
    suffix = "/api/v1/process"
    if trimmed.lower().endswith(suffix):
        return trimmed[: -len(suffix)] or trimmed
    return trimmed
...
def get_bisheng_settings() -> BishengSettings:
    ...
    base_url_raw = _bisheng_setting("BISHENG_BASE_URL", config_key="base_url", default="http://localhost:3001") or ""
    base_url = _sanitize_base_url(base_url_raw)
