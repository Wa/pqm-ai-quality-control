...
def _build_run_manifest(
    ...
    for fname in sorted(exam_txt_files, key=lambda value: value.lower()):
        ...
        for chunk_index, piece in enumerate(chunks):
            entry_id += 1
            prompt_text = SPECIAL_SYMBOLS_WORKFLOW_SURFACE.build_chunk_prompt(piece)
            num = chunk_index + 1
...
def run_special_symbols_job(
    ...
    kb_name_dyn = SPECIAL_SYMBOLS_WORKFLOW_SURFACE.knowledge_base_name(session_id)
    kid = None
    try:
        ...
        if kid and reference_txt_files:
            res = kb_sync_folder(
                base_url=settings.base_url,
                api_key=settings.api_key or None,
                knowledge_id=int(kid),
                folder_path=reference_txt_dir,
                clear_first=False,
                chunk_size=1000,
                chunk_overlap=0,
                separators=["\n\n", "\n"],
        ...
        elif not reference_txt_files:
            emitter.warning("未检测到基准文件文本，继续执行但可能影响比对效果")
        else:
            emitter.warning("KB 创建或查找失败，继续执行但可能影响检索效果")
    except Exception as error:
        report_exception("特殊特性基准文件KB同步失败", error, level="warning")

    emitter.set_stage("warmup")
    try:
        warmup_prompt = SPECIAL_SYMBOLS_WORKFLOW_SURFACE.warmup_prompt or "预热：请回复 'ready for symbols'。"
        call_flow_process(
            base_url=settings.base_url,
    ...
    for name in sorted(exam_txt_files, key=lambda value: value.lower()):
        ...
        for index, piece in enumerate(chunks, start=1):
                    ...
                    publish(
                        {
                            "status": "paused",
                            "stage": "paused",
                            "message": f"暂停中：等待恢复（{name} 第{index}/{total_parts}）",
                        }
                    )
                    time.sleep(1)
                    try:
            ...
            log_llm_metrics(
                ...
                session_id,
                {
                    "ts": datetime.now().isoformat(timespec="seconds"),
                    "engine": "bisheng",
                    "model": f"special_flow:{settings.flow_id or 'unknown'}",
        ...
        try:
            ...
            summarize_with_ollama(initial_results_dir, output_root, session_id, name_no_ext, full_out_text)
        except Exception as error:
