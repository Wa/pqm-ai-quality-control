...
SPECIAL_SYMBOLS_CHUNK_PROMPT_PREFIX = (
    ...
    "对于每一处不一致，请提供具体的条目（即特征、特性、过程或产品）、基准文件名和工作表名（如果有工作表名信息）、"
    "基准文件中的特殊特性分类（★、☆、/）、待检查文件名和工作表名（如果有工作表名信息）、和待检查文件中的特殊特性分类（★、☆、/）。\n"
    "\n要求和注意事项："
    "1) 请将待检查文件中的所有条目进行逐一检查。"
    "2) 尽量保留可用于追溯定位的原文线索（如文件名、工作表名等）于相应字段中。"
    "3) 斜杠，即/，不总是代表特殊特性分类，有时斜杠代表该处无内容，请结合上下文判断。"
    "4) 条目的具体文字表述可能有一些差异，你需要根据意思是判断是否是同一个条目。"
    "5) 如果一个条目仅在基准文件中出现，而在待检查文件中没有出现，则不需要列出。"
    "6) 同样地，如果一个条目仅在待检查文件中出现，而在基准文件中没有出现，则不需要列出。"
    "7) 不需要列出一致的条目。"
    "以下是待检文件的一部分：\n"
)
