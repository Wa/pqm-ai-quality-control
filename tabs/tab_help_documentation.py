...
def render_settings_section():
    ...
    st.markdown("""
    本页提供账号会话管理与模型服务连通性检查，帮助确认后台依赖是否可用。
    ...
    ### 🔗 连接状态
    - 自动检测内网 Ollama 推理服务是否可用。
    - 若已配置 OpenAI API Key，也会尝试联网连通性检测。
    - 若出现离线，请检查服务是否启动、网络/代理是否可用。

    ### 📚 相关文档
    - 快速跳转到 Ollama 与 OpenAI 的官方文档、参数说明及网关信息。

    > 当前版本的设置页仅用于监控与参考，暂不提供模型或推理参数调整。
    """)
