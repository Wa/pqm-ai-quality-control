...
def render_special_symbols_check_section():
    ...
    st.markdown("""
    该功能正在进行全面升级，以与“企业标准检查”工作流保持一致。新版将：

    - 采用统一的工作流编排与后台任务机制，避免在前端拼接超长提示词；
    - 提供与企业标准检查相同的文件管理、任务状态与结果下载体验；
    - 精简提示词，并通过后台服务完成大模型交互。

    在升级完成之前，页面仅展示占位信息，不再触发旧版流程。如需参考新的交互模式，可先体验“🏢 企业标准检查”标签页。
    """)
