...
def render_parameters_check_tab(session_id):
    ...
    # Base directories for each upload box - using centralized config (same as consistency check)
    BASE_DIRS = {
    ...
    with col_info:
        ...
        with col_bulk2:
            if st.button("🔄 刷新文件列表", key=f"parameters_refresh_file_list_{session_id}"):
